...
/*   Updated: 2021/02/26 23:46:08 by drissi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	store_rays(int strip_id, float ray_angle)
{
	g_ray.horz_distance = g_ray.horz_hit ? distance(g_player.x, g_player.y,
		g_ray.x_hit_horz, g_ray.y_hit_horz) : (float)INT_MAX;
	g_ray.vert_distance = g_ray.vert_hit ? distance(g_player.x, g_player.y,
		g_ray.x_hit_vert, g_ray.y_hit_vert) : (float)INT_MAX;
	...
	g_rays[strip_id].right = ray_angle < (M_PI / 2) ||
		ray_angle > (1.5 * M_PI);
